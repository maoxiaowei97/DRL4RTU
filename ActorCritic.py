...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.ref_fea = np.load('/data/maodawei/DRL4RTP-0725/data/similar_routes/similar_routes/ref_fea.npy', allow_pickle=True)
        self.ref_sigma = np.load('/data/maodawei/DRL4RTP-0725/data/similar_routes/similar_routes/ref_sigma.npy', allow_pickle=True)
        self.similar_route_lstm = nn.LSTM(input_size=hidden_dim,
                            hidden_size=hidden_dim,
                            num_layers=1, batch_first=True)
        self.similar_route_mlp =  nn.Sequential(
        ...
        self.route_proj = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim + 5 + 10),
        )
    ...
    def recurrence_eta(self, h_in, step, next_node_idxs, V_mask, start_fea, context, last_node, decode_type):
        ...
        g_l, h_out = hy, (hy, cy)
        route_state = torch.cat([start_fea.reshape(B, -1).float(), last_node, current_node_input.float(), g_l], dim=1)#出发节点特征，下一节点特征，所有节点特征， 加一个当前节点特征，预测时间间隔
    ...
    def similar_route_learning(self, o_geo, d_geo, step, route_state, decode_type):

        B = o_geo.shape[0]
        K = self.ref_fea.shape[3]
        ...
        if decode_type == 'mle':
            ...
            sigma_log_prob = sigma_dist.log_prob(component_sample_prob)  # sampled value may be negative, change to log distribution
    ...
    def forward(self, start_fea, decoder_input, embedded_inputs, hidden, context, V_reach_mask, V_mask, pred_len, first_node_input, start_geo, V_geo, decode_type):
        ...
        sigma_label_resort = torch.zeros([decoder_input.shape[0], V_mask.shape[1]]).to(decoder_input.device) #每个样本，在每个标签位的标准差标签
        ...
        save_pi = []
        o_geo = start_geo
        d_geo = torch.zeros_like(o_geo)
        last_node = first_node_input.float()
        current_eta = 0

        if decode_type == 'mle':
            for i in steps:
                hidden, log_p, probs, mask = self.recurrence(decoder_input, hidden, mask, i, idxs, context, V_mask, start_fea, decode_type)
                ...
                d_geo = V_geo[torch.arange(idxs.shape[0]), idxs]
                hidden_eta, eta_duration_pred, m, eta_mu, eta_pi, eta_sigma, route_state, last_node = self.recurrence_eta(hidden_eta,  i, idxs, V_mask, start_fea, context, last_node,  decode_type)  #输出eta的类别分布，隐藏状态，先regression
                sigma_argmax, similar_routes_label = self.similar_route_learning(o_geo, d_geo, i, route_state, decode_type) # 输入起点，终点，间隔节点数量，函数内取出相似路线的特征，映射到标准差的高斯分布，再取均值得到标准差。还需要取出每个样本根据路线计算的平均标准差作为标签

                # current_eta = current_eta + eta_duration_pred
                eta_duration_pred, sigma_argmax = self.gated_fusion(eta_duration_pred, sigma_argmax) # sigma是针对到达时间的标准差，不是节点间的
                current_eta = current_eta + eta_duration_pred
                save_mu.append(eta_mu)
                save_pi.append(eta_pi)
                save_sigma.append(eta_sigma)
                # pred_eta_list.append(current_eta)
                # pred_sigma_list.append(sigma_argmax)
                ...
                sigma_label_resort[[x for x in range(0, idxs.shape[0])], idxs.tolist()] = similar_routes_label.squeeze(1).float()
            ...
            return (torch.stack(outputs, 1), torch.stack(selections, 1), eta_resort, torch.stack(eta_selections, 1), sigma_resort,
            ...
            for i in steps:
                ...
                hidden_eta, m, pi, route_state, last_node  = self.recurrence_eta(hidden_eta,  i, idxs, V_mask, start_fea, context, last_node, decode_type)  #输出时间间隔的m, pi
                sigma_sample, sigma_log_prob = self.similar_route_learning(o_geo, d_geo, i, route_state, decode_type)

                eta_sample_duration, eta_log_prob = self.decode_eta(
                    m,
                    pi
                )
                eta_sample_duration, sigma_sample = self.gated_fusion(eta_sample_duration, sigma_sample)
                current_eta = current_eta + eta_sample_duration
                # current_eta, sigma = self.gated_fusion(current_eta, sigma_sample)
            ...
            return (torch.stack(selections, 1), torch.stack(eta_selections, 1), torch.stack(sigma_selections, 1),
                    torch.stack(order_log_probs, dim=1), torch.stack(eta_log_probs, dim=1), torch.stack(sigma_log_probs, dim=1), torch.stack(rt_log_probs, dim=1))
...
class DeepRoute(nn.Module):
    def __init__(self, args={}):
        ...
        self.first_node_embedding = nn.Linear(in_features=self.sort_x_size + self.t_size, out_features=self.hidden_size, bias=False)
    ...
    def forward(self, V, V_reach_mask, start_fea, cou_fea, pred_len, first_node, start_geo, V_geo,  decode_type):
        ...
        V_avg_t = V_dis / cou_speed.unsqueeze(1).unsqueeze(1).repeat(1, N, 1)  # [B*T, N, 2]
        ...
        if decode_type == 'mle':
            (pointer_log_scores, order_arg, eta_resort, eta_select, sigma_resort, sigma_select, sigma_label_resort, m, sigma, pi) = self.decoder(start_fea, decoder_input, inputs, dec_init_state, enc_h, V_reach_mask.reshape(-1, N), V * mask, pred_len, first_node_input, start_geo, V_geo, decode_type)
            ...
            (order_samples, eta_samples, sigma_samples, order_log_probs, eta_log_probs, sigma_log_probs, rt_log_probs) = self.decoder(start_fea, decoder_input, inputs, dec_init_state, enc_h, V_reach_mask.reshape(-1, N), V * mask, pred_len, first_node_input, start_geo, V_geo, decode_type)
    ...
    def model_file_name(self):
        ...
        file_name = f'{file_name}.drl4rtp{time.time()}.csv'
        return file_name
...
def save2file(params):
    ...
    file_name = ws + f'/output/rtp_1119/{params["model"]}.csv'
    # 写表头
    head = [
        # data setting
        'dataset', 'min_task_num', 'max_task_num', 'eval_min', 'eval_max', 'state', 'time_step_reward', 'time_policy_ratio','target',
        # mdoel parameters
        'model', 'hidden_size', 'rl_ratio', 'trace_decay', 'route_metric', 'route_reward','point_estimation', 'reward_type',
        # training set
        'num_epoch', 'batch_size', 'lr', 'wd', 'early_stop', 'is_test', 'log_time', 'weak_label', 'use_mis', 'cred', 'gated_fusion',
        'lsd', 'lmd', 'krc', 'hr@1', 'hr@2', 'hr@3', 'ed',
        # metric result
        'mape', 'mae', 'rmse', 'acc_eta@10', 'acc_eta@20', 'acc_eta@30', 'acc_eta@40', 'acc_eta@50', 'acc_eta@60', 'picp', 'mis', 'interval'
    ]
