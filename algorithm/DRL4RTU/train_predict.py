...
def get_eta_result(pred, label, label_len, label_route,  pred_pointers, eta_label_len, route_label_all, sigma):
    ...
    for i in range(B):
        if label_len[i].long().item() != 0:
            eta_pred_result[i][:label_len[i].long().item()] = pred[i][label_route[i][:label_len[i].long().item()].long()]
...
def eta_maemis_loss_calc(time_label, eta_label_len, eta, sigma, label_route, sigma_label, params):
